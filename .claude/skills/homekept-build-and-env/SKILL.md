---
name: homekept-build-and-env
description: >-
  Recreate the HomeKept build environment from scratch and run builds/tests
  locally, including the traps a newcomer hits. Load this when setting up the
  repo, when a build or test command fails, when unsure how to verify a change
  locally, or when confused about the Docker/Testcontainers requirement.
  Triggers: "how do I build", "gradlew", "bun install", "compile only", "tests
  won't run", "Docker required", "Testcontainers", "route tree", "frozen
  lockfile", setup, environment, toolchain, versions.
---
...
- **Backend config is a hand-write boundary.** `application.yml` and
  ...
  not exist, STOP (see `homekept-change-control`), do not add it.
...
.claude/    Agents, the /ship command, skills (this library), homekept-feature.md
```

## When NOT to use this skill / use a sibling instead

- Deploying / running against a live stack → `homekept-run-and-operate`.
- The config keys and env vars themselves → `homekept-config-and-flags`.
- What to test and what counts as evidence → `homekept-validation-and-qa`.
- Diagnosing a specific failing build/runtime symptom → `homekept-debugging-playbook`.
