---
name: homekept-external-positioning
description: >-
  What HomeKept may claim publicly and how: the advertising-law rails
  (Competition Act, CASL), what is genuinely novel vs commodity, what must be
  proven before it is claimed, and the sanctioned social-proof mechanism. Load
  this when writing marketing copy, a landing page, an email/SMS campaign, a
  testimonial, or any outward-facing claim, or when deciding whether a number
  can be stated publicly. Triggers: marketing, landing page, testimonial,
  review, social proof, "can we say", claim, Competition Act, CASL, positioning,
  differentiator, launch messaging.
---
...
- **Testimonials must be real and disclosed.** The sanctioned social-proof
  ...
  `homekept-domain-reference`). **Never a Google review**: incentivized or gated
  reviews violate Google policy and Competition Act guidance. Do not build a flow
  that trades a discount for a review.
...
- **No contracts at the door.** Door-knocking invites people to book online only;
  it never closes a sale on the doorstep (see `homekept-domain-reference`).
...
- **Capability claims** — only for what is built. The product degrades
  ...
  are wired and verified (see `homekept-go-live-campaign`).
...
- Internal docs and PR/commit writing → `homekept-docs-and-writing`.
...
- In-product copy rules (em dashes, empty states, contrast) →
  `homekept-docs-and-writing` + `homekept-change-control`.
