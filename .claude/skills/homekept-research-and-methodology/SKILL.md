---
name: homekept-research-and-methodology
description: >-
  Where HomeKept could advance beyond the obvious, and the discipline that turns
  a hunch into an accepted result here. Load this when proposing a genuinely new
  direction (a data-asset play, an ops-efficiency idea, a differentiating
  feature), when evaluating whether an idea is worth building, or when running an
  experiment and deciding if it earned adoption. Also the meta-guide for
  maintaining this skill library. Triggers: "new idea", "could we", frontier,
  data moat, Home Health Score, ops efficiency, experiment, hypothesis, "is this
  worth it", research, methodology, advance the product.
---
...
This is the most speculative skill in the library. It never routes around change
control: any idea that becomes code goes through the crew and the gates
(`homekept-change-control`). "Beyond state of the art" here does not mean novel
computer science; it means a durable advantage in a home-maintenance business.
...
3. **Survive adversarial refutation.** The project already runs this culture: the
   `safety-reviewer` tries to break a load-bearing change before it merges. Apply
   the same to an idea — assign someone (or an agent) to refute it. If it
   survives a genuine attempt to kill it, it is stronger.
4. **The idea lifecycle.** experiment (behind a flag / off the main path) →
   measured against the pre-stated threshold → **adopted** (goes through change
   control into the product) OR **retired** (documented in
   `homekept-failure-archaeology` so it is not re-tried blindly). No idea stays
   in limbo.
...
- Every skill: YAML frontmatter (`name` + a trigger-rich `description`), imperative
  runbook voice, ground-truth-only, "when NOT to use / use sibling instead", and
  a "Provenance and maintenance" section with re-verify commands.
...
- **No oversell:** unproven things stay labeled candidate/open. Nothing may
  contradict `CLAUDE.md` or route around change control.
- When the repo changes a load-bearing fact, update the owning skill and its
  provenance block in the same spirit as the api-contract discipline.

## When NOT to use this skill / use a sibling instead

- Proving a specific invariant holds → `homekept-proof-and-analysis-toolkit`.
- What may be *claimed publicly* about a frontier → `homekept-external-positioning`.
- The business facts a frontier builds on → `homekept-domain-reference`.
- Getting an adopted idea shipped → `homekept-change-control` + `homekept-feature.md`.
...
- Revisit this skill whenever a frontier is adopted or retired; move retired ones
  to `homekept-failure-archaeology`.
