---
name: homekept-proof-and-analysis-toolkit
description: >-
  First-principles "prove it, don't assume it" recipes for HomeKept: how to turn
  a claim ("ownership is safe", "money is cents", "the transition is legal",
  "config binds", "contrast passes", "the webhook works") into a repeatable
  measurement, each with a worked example from this repo's history. Load this
  when you need to demonstrate a property actually holds rather than assert it,
  when reviewing a load-bearing change, or when someone says "it works" without
  evidence. Triggers: "prove it", "how do I know it works", "is this actually
  safe", verify invariant, demonstrate, measurement, worked example, IDOR test,
  contrast ratio, binding test.
---
...
Use this with `homekept-validation-and-qa` (what to test) and
`homekept-diagnostics-and-tooling` (the instruments). This skill is the *epistemics*:
how to design the measurement so a pass actually means something.
...
- **Worked example:** portfolio Phase 1 (#132) resolves every `propertyId`
  ...
  both a read and a write and asserts 404. The safety review traced *every*
...
- **Measurement:** the DTO field is an `int`/`long` of cents; assert the exact
  ...
  `homekept-diagnostics-and-tooling` recipe 3).
...
- **Worked example:** the #122 reskin shipped three sub-AA combos, fixed by
  ...
  regressions, ratios, and hex values live once in `homekept-failure-archaeology`
  #3. Text on the amber accent is always dark ink `#11201a`.
...
- The catalog of what must be tested → `homekept-validation-and-qa`.
- The runnable instruments (scripts, curl, CI logs) → `homekept-diagnostics-and-tooling`.
- Turning a proven result into an accepted project change → `homekept-research-and-methodology`.
- The specific past incidents referenced → `homekept-failure-archaeology`.

## Provenance and maintenance

Verified 2026-07-06 against the tests and commits cited. Re-verify with the
Provenance commands in `homekept-validation-and-qa` and
`homekept-failure-archaeology`. Add a recipe whenever a new class of invariant
