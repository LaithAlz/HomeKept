---
name: homekept-docs-and-writing
description: >-
  How to maintain HomeKept's documents of record and write in the house style:
  which doc is authoritative for what, the api-contract discipline, the design-
  token source of truth, and the copy rules (plain language, no em dashes, no
  fabrication, amber-on-dark-ink). Load this when writing or updating docs, a PR
  description, user-facing copy, or a commit message, or when unsure which doc
  owns a fact. Triggers: "update the docs", "which doc", "api contract", "PR
  description", "commit message", "copy style", "house style", em dash, tone,
...
| `CLAUDE.md` | The agent guide + non-negotiables (the top-level index) |
| `.claude/skills/homekept-feature.md` | The mandatory change procedure |

**One home per fact.** If a fact lives in a doc of record, reference it — do not
copy it into another doc where it will drift. (These skills follow the same
rule: each fact has one owner skill; siblings cross-reference.)
...
field means documenting its exact shape there. `spec-guardian` checks diffs
against it. A frontend hook and the contract must agree (this is where the #17
register-vs-contract conflict came from — see `homekept-failure-archaeology`).
...
- **No em dashes** in customer copy: use periods, commas, colons; code comments
  are exempt. (The rule and its rationale live once in `homekept-change-control`
  non-negotiable #8; this is the pointer.)
- **No fabricated data or social proof.** Honest empty states over fake numbers.
  See `homekept-external-positioning` for the legal reason.
...
- **Trades-safe wording** in anything describing the service (see
  `homekept-domain-reference`): "visual inspection / performance observation",
  "not a certified inspection", never implying licensed-trade work.

## Templates

**PR description:** What (one paragraph) / Why (one paragraph) / Scope (what is
and is not touched) / Reviews (paste the spec-guardian, and safety/copy verdicts
where they ran). Wait for CI green, then stop for a human to merge.
...
- Public marketing claims and legal advertising rails → `homekept-external-positioning`.
- The business facts the docs describe → `homekept-domain-reference`.
- The review gates that enforce doc-accuracy → `homekept-change-control`.
- Writing/maintaining these skill files themselves → follow this skill's style,
  and the meta-notes in `homekept-research-and-methodology`.
