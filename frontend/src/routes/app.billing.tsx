...
function BillingPage() {
  ...
  return (
    <div className="px-6 py-10 md:px-10">
      ...
      <p className="mt-2 max-w-2xl text-muted-foreground">Your plan, payment method, and invoice history.</p>
        ...
        <div className="rounded-3xl border border-border bg-card p-6 lg:col-span-2">
          ...
          <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-sm text-muted-foreground">Billed monthly · renews June 1</div>
            </div>
          ...
          <div className="mt-5 flex gap-2">
            <Button size="sm" variant="outline">Switch to annual (save {fmt(price * 12 * 0.15)})</Button>
            ...
            <tbody>
              {invoices.map((i) => (
                <tr key={i.id} className="border-t border-border">
                  <td className="px-4 py-3">{new Date(i.date).toLocaleDateString("en-CA", { month: "short", day: "numeric", year: "numeric" })}</td>
                  ...
                  <td className="px-4 py-3"><span className="rounded-full bg-emerald-500/10 px-2 py-0.5 text-xs text-emerald-700 dark:text-emerald-300">{i.status}</span></td>
                  <td className="px-2 py-3"><Button size="icon" variant="ghost" className="h-7 w-7"><Download className="h-3.5 w-3.5" /></Button></td>
                </tr>
              ))}
            </tbody>
          </table>
