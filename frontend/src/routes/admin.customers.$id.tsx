...
export const Route = createFileRoute("/admin/customers/$id")({
  head: ({ params }) => ({
    meta: [
      { title: `Customer #${params.id} — HomeKept Admin` },
 ...
 * rid of the sliding side-panel"), later renamed from "Subscriber" to "Customer" in
 * the UI (issue: "I don't want a subscribers page, I want a customers page" — the
 * backend domain term `subscriber` is unchanged, this is UI language only). The
 * record has six-plus sections an operator reads, cross-references and acts on — it
 * deserves a URL and real width, not `sm:max-w-md`. `/admin/customers?id=N` (used by
 * the dashboard and the routes dispatch board) redirects here — see the `beforeLoad`
 * on `/admin/customers`.
 */
function CustomerDetailPage() {
  ...
  // switching from one customer to another), and announces it to screen readers —
  ...
  if (!validId || is404) {
    body = <CustomerNotFound headingRef={headingRef} />;
    crumbLabel = "Not found";
  } else if (query.isLoading || !query.data) {
    body = <CustomerLoading headingRef={headingRef} />;
    crumbLabel = "Loading…";
  } else if (query.isError) {
    body = <CustomerLoadError headingRef={headingRef} onRetry={() => void query.refetch()} />;
    crumbLabel = "Customer detail";
  } else {
    body = <CustomerDetailView detail={query.data} headingRef={headingRef} />;
    crumbLabel = subscriberFullName(query.data) || `Customer #${query.data.id}`;
  }
...
function CustomerLoading({ headingRef }: { headingRef: HeadingRef }) {
  return (
    <div className="py-16">
      ...
      <PanelLoading label="Loading customer." className="justify-center" />
    </div>
  );
}

function CustomerLoadError({
  headingRef,
  onRetry,
}: {
...
function CustomerNotFound({ headingRef }: { headingRef: HeadingRef }) {
  return (
    <div className="mx-auto max-w-md py-20 text-center">
      ...
      <p className="mt-3 text-sm text-muted-foreground">
        We couldn't find that customer. It may have been removed or the link may be incorrect.
      </p>
      <div className="mt-8">
        <Button asChild>
          <Link to="/admin/customers">
...
function CustomerDetailView({
  ...
  const title = hasName ? fullName : `Customer #${detail.id}`;
  ...
  // heading is just "Customer #N" (no name on file), the line under it is more
...
function describeSubscriptionActionError(err: unknown): string {
  if (err instanceof ApiError) {
    if (err.code === "NO_BILLING_ACCOUNT") {
      return "This customer has no Stripe subscription yet.";
    }
