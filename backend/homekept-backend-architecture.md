...
- `VisitEvent` — id, visit_id, event_type, payload (JSONB), by_user_id, source (ADMIN / CUSTOMER / TECHNICIAN / SYSTEM), created_at — the visit's own activity log (V14 migration), mirroring `subscription_event`. Records reschedule (before/after times), technician assignment/change, and cancellation. Newest-first, capped read via `GET /api/admin/visits/{id}/events`. This is what replaced the old "reschedule creates a new visit" history model — see the state-machine note below and Part 4 §4.2
