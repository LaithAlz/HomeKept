...
| `GET /api/tech/visits/{id}/notes?cursor=&limit=` | this visit's threaded operational notes, newest first, cursor-paginated (default page size 20, max 100 — same `cursor`/`limit` convention as `GET /api/admin/visits`): `{ notes: [{ id, body, createdAt, authorUserId, authorFirstName, authorLastName }], nextCursor }` — `nextCursor` is the `id` to pass as `?cursor=` for the next page, `null` when there is no more. Same `visit_note` rows the admin console reads/writes via `GET`/`POST /api/admin/visits/{id}/notes` (see below); NOT the same thing as `completionNotes`/`materialsNotes` (single fields set once at completion). Visit not found, or not assigned to this technician → `404` (indistinguishable, per the ownership-failure rule) |
| `POST /api/tech/visits/{id}/notes` | `{ body }` (1–2000 chars) — adds a note; author is always this authenticated technician, never a request field. → `201` with the created note. Same `404` rule as the GET above. No delete/edit endpoint exists for a note: it is deliberately append-only (same as `visit_event`/`subscription_event`) — a correction is a new note, not erasing the old one |
| `GET /api/tech/properties/{propertyId}/notes?cursor=&limit=` | this property's threaded operational notes, newest first, cursor-paginated (same shape/convention as the visit notes above): `{ notes: [...], nextCursor }`. Same `property_note` rows the admin console reads/writes via `GET`/`POST /api/admin/properties/{propertyId}/notes`. Standing context about the home ("dog in the yard", "gate sticks"), NOT a single visit's own notes. Property not found, OR this technician does not have a qualifying assignment at it — SCHEDULED, IN_PROGRESS, COMPLETED, or INCOMPLETE; a CANCELLED (or legacy RESCHEDULED) visit does NOT qualify, so cancelling a technician's only visit at a property revokes their access to it → `404` (indistinguishable from "property doesn't exist") |
| `POST /api/tech/properties/{propertyId}/notes` | `{ body }` (1–2000 chars) — adds a note; author is always this authenticated technician. → `201` with the created note. Same `404` rule as the GET above. Plaintext only — never accept a lockbox/alarm code or key location here; those belong exclusively in the encrypted `access_notes` column, which has no write endpoint at all |
...
| `GET /api/admin/subscribers?cursor=` | subscriber list w/ status, plan, MRR, and customer identity — `firstName`, `lastName`, `email`, `phone` (resolved from the identity domain via a single batched query per page, never N+1; customer PII, safe here only because this endpoint is ADMIN-gated — `phone` is frequently `null` since it isn't captured at account creation). `mrrCents` is never `null`: it is the plan's monthly price ONLY while the subscriber's status is currently-paying revenue (`SubscriberStatus.isBilling()` — ACTIVE only), and `0` otherwise (CANCELLED, PAUSED, PAYMENT_ISSUE, PENDING_ACTIVATION all read `$0`). `planPriceCents` is the plan's list price regardless of billing status (`null` if no plan chosen yet) — kept separate so the row can still show e.g. "Complete, $169/mo" as a plan attribute on a $0-MRR (e.g. cancelled) row |
| `GET /api/admin/subscribers/{id}` | detail incl. property, Stripe links, and customer identity — `firstName`, `lastName`, `email`, `phone` (same identity-domain resolution and PII caveat as the list above). `property` includes `propertyId` (targets the SKU update below) and the SKU sheet fields — `hvacFilterSizes`, `smokeCoDetectorModels`, `humidifierModel`, `waterHeaterAgeYears`, `waterHeaterFlushEligible` (all `null` until captured; technician-prep data per docs/pricing-and-visits.md §Materials). `mrrCents`/`planPriceCents` — same split and semantics as the list above |
...
| `GET /api/admin/properties/{propertyId}/notes?cursor=&limit=` | the property's threaded operational notes, newest first, cursor-paginated (default page size 20, max 100 — same `cursor`/`limit` convention as `GET /api/admin/visits`): `{ notes: [{ id, body, createdAt, authorUserId, authorFirstName, authorLastName }], nextCursor }` (`property_note` table, V18) — standing knowledge about the home ("dog in the yard", "gate sticks, lift while pushing"), as distinct from a single visit's own notes. An admin has no status restriction (unlike the technician endpoint below). Same rows the assigned technician reads/writes via `GET`/`POST /api/tech/properties/{propertyId}/notes`. Always plaintext — never the encrypted `access_notes` column. Unknown `propertyId` → 404 |
| `POST /api/admin/properties/{propertyId}/notes` | `{ body }` (1–2000 chars) — adds a note; author is always the authenticated admin, never a request field. → `201` with the created note. Never accept a lockbox/alarm code or key location here — those belong exclusively in the encrypted `access_notes` column, which has no write endpoint at all. No delete/edit endpoint: append-only, same reasoning as the visit notes above |
| `GET /api/admin/visits?status=&subscriberId=&cursor=&limit=` | cursor-paginated visit list (newest first; mirrors the bookings pagination style; also backs the admin Routes day view, which calls this filtered to `status=SCHEDULED` and groups the rows by technician client-side): `[{ id, subscriberId, propertyId, technicianId, scheduledFor, durationMinutes, actualDurationMinutes, materialsCostCents, status, type, completedAt, createdAt, customerFirstName, customerLastName, customerPhone, propertyStreetAddress, propertyCity }]`. The five `customer*`/`property*` fields are resolved via two batched queries per page (identity domain via `UserQueryService.findAdminContactsByIds`, property domain via `PropertyService.findByIds`) — never one query per row — and are `null` only if the referenced subscriber/property is unexpectedly missing; customer PII, safe here only because this endpoint is ADMIN-gated (same caveat as the subscriber list above). `subscriberId` filters to one subscriber's visits (backs the admin customer page's visit list) and composes with `status` — either, both, or neither may be supplied; an unknown `subscriberId` simply yields an empty page (no existence check). Invalid `status` → 400 |
| `POST /api/admin/visits` | `{ subscriberId, scheduledFor, durationMinutes, serviceIds[], technicianUserId? }` — `technicianUserId`, when present, must resolve to a real user with the TECHNICIAN role (checked via the identity domain's service) or the request is rejected with `400 INVALID_REQUEST`; `visit.technician_id` doubles as an authorization principal for that visit's and its property's notes, so an unvalidated id would be a standing-access bug, not just a display bug |
...
| `PATCH /api/admin/visits/{id}` | reschedule (updates `scheduledFor`/technician on the SAME visit row IN PLACE — no replacement visit is created — and records a `RESCHEDULED` `visit_event`) / cancel (records a `CANCELLED` `visit_event`) / assign technician (`technicianUserId`; must resolve to a real TECHNICIAN, else `400 INVALID_REQUEST`; records a `TECHNICIAN_ASSIGNED` `visit_event` only when the technician actually changes) / unassign technician (`unassignTechnician: true` — the only way to clear `technicianId` back to `null` through this API; records a `TECHNICIAN_ASSIGNED` event with `to: null` if a technician was assigned). `technicianUserId` and `unassignTechnician` are mutually exclusive — `400 INVALID_REQUEST` if both are supplied. Every `visit_event` this produces has `source=ADMIN` and `byUserId` = the authenticated admin's user id. `scheduledFor`, when present, must be in the future — `400 VALIDATION_FAILED` otherwise. 404 if missing; 409 `ILLEGAL_STATE_TRANSITION` if the visit isn't in a state that permits the requested op (e.g. rescheduling a CANCELLED visit, cancelling a COMPLETED visit — the message names the visit's actual status, not a status transition) |
...
| `GET /api/admin/visits/{id}/notes?cursor=&limit=` | the visit's threaded operational notes, newest first, cursor-paginated (default page size 20, max 100 — same `cursor`/`limit` convention as `GET /api/admin/visits`): `{ notes: [{ id, body, createdAt, authorUserId, authorFirstName, authorLastName }], nextCursor }` — unlike the events log above, the author IS resolved to a name (batched, never per-row), since a note's whole point is a readable log of who observed what. Same rows the assigned technician reads/writes via `GET`/`POST /api/tech/visits/{id}/notes`. NOT the same thing as `completionNotes`/`materialsNotes` (single fields set once at completion) — this is a log, any number of entries, and every one of them stays reachable by paging (there is no fixed cap past which a note becomes unreachable). Unknown visit → `404` |
| `POST /api/admin/visits/{id}/notes` | `{ body }` (1–2000 chars) — adds a note; author is always the authenticated admin, never a request field. → `201` with the created note. No delete/edit endpoint: this is deliberately append-only (same as `visit_event`/`subscription_event`) — a correction is a new note, not erasing the old one |
...
| `GET /api/admin/dashboard` | aggregate metrics for the admin home / operational dashboard (#43): `{ activeSubscribers, mrrCents, pendingWalkthroughs, upcomingVisits }`. `mrrCents` sums the current monthly price across ACTIVE subscribers only (unchanged: every ACTIVE subscriber already reads `isBilling() = true`, so this aggregate agrees exactly with the per-subscriber `mrrCents` field above); `pendingWalkthroughs` counts PENDING (unconfirmed) bookings; `upcomingVisits` counts SCHEDULED visits with `scheduledFor` at or after now. No "at-risk subscribers" field — there is no backing status/column for that concept yet |
| `GET /api/admin/services` | every catalog service, active AND archived (sorted tier class then name — same order as the public picks menu), so the console can show what's archived: `[{ id, name, category, tierClass, defaultDurationMinutes, aLaCartePriceCents, description, isFreeWithEveryVisit, active, createdAt, updatedAt }]`. `GET /api/catalog/picks` (public) still only returns `active = true` services |
| `POST /api/admin/services` | `{ name, category, tierClass, defaultDurationMinutes, aLaCartePriceCents?, description, isFreeWithEveryVisit? }` — `category` must be one of `HVAC`/`PLUMBING`/`EXTERIOR`/`SMART_HOME`, `tierClass` one of `BASIC`/`MEDIUM`/`PREMIUM` (the `service` table's CHECK constraints) — a bad value is `400 VALIDATION_FAILED`, never a 500. `aLaCartePriceCents` is nullable (a standing item that is never sold à la carte); when present it must be a positive integer cent amount. `isFreeWithEveryVisit` defaults `false` when omitted. New services always start `active = true`. → `201` with the created service |
| `PATCH /api/admin/services/{id}` | `{ name?, category?, tierClass?, defaultDurationMinutes?, aLaCartePriceCents?, description?, isFreeWithEveryVisit? }` — every field optional; a field omitted or `null` leaves that column unchanged (matches the SKU-sheet PATCH's partial-update semantics). Never touches `active` — see the archive/restore actions below. Unknown `id` → `404`; a bad `category`/`tierClass` → `400 VALIDATION_FAILED` |
| `POST /api/admin/services/{id}/archive` | sets `active = false`. Never a hard delete: `visit_service`, `plan_tier_service`, and `visit_template_service` all reference `service` with `ON DELETE RESTRICT`, and archiving is the intended mechanism — existing `visit_service` rows keep pointing at the same service id/name (visit history is unaffected), it simply drops out of `GET /api/catalog/picks` for future bookings. Idempotent (archiving an already-archived service is a no-op `200`). Unknown `id` → `404` |
| `POST /api/admin/services/{id}/restore` | sets `active = true`. Idempotent. Unknown `id` → `404` |
| `POST /api/admin/plan-tiers/{planTierId}/services` | `{ serviceId, frequencyPerYear }` — adds a service to a plan tier's composition (a `plan_tier_service` row). `plan_tier_service` has a composite primary key (`plan_tier_id`, `service_id`); adding a pair that already exists is `409 CONFLICT` (naming the PATCH endpoint below), never a 500 from the constraint violation. Unknown `planTierId` or `serviceId` → `404`; non-positive `frequencyPerYear` → `400 VALIDATION_FAILED`. → `201 { planTierId, serviceId, serviceName, serviceTierClass, frequencyPerYear }`. Never writes plan tier pricing (`monthlyPriceCents`, `annualPriceCents`, `visitsPerYear`, `includedPicksPerYear`, `maxPremiumPicksPerYear`, any Stripe price id) — those stay migration + Stripe only |
| `PATCH /api/admin/plan-tiers/{planTierId}/services/{serviceId}` | `{ frequencyPerYear }` (required — this action has exactly one field) — changes an existing composition row's frequency. `404` if the plan tier, the service, or the (planTierId, serviceId) composition row itself doesn't exist; non-positive `frequencyPerYear` → `400 VALIDATION_FAILED` |
| `DELETE /api/admin/plan-tiers/{planTierId}/services/{serviceId}` | removes a service from the plan tier's composition. `404` if the plan tier, the service, or the composition row doesn't exist (not a silent no-op — matches the ownership/existence-failure convention used elsewhere, e.g. `DELETE /api/app/todos/{id}`). → `204` on success |
