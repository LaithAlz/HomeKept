...
| `POST /api/admin/subscribers/{id}/pause` · `POST /api/admin/subscribers/{id}/resume` | → `200 { status, currentPeriodEnd }` — admin-triggered version of the customer self-serve pause/resume (same Stripe calls and guards): pause requires ACTIVE, resume requires PAUSED, else `409 ILLEGAL_STATE_TRANSITION`; unknown subscriber → `404`; no Stripe subscription yet → `409 NO_BILLING_ACCOUNT`. Unlike the customer self-serve pause/resume, these two require `Content-Type: application/json` (send `{}` — there is nothing to bind, this is purely a CSRF-hardening content-type gate) → `415` without it |
