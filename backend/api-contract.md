...
| `POST /api/admin/subscribers/{id}/cancel` | `{ reason, immediately? }` (`reason` required, 1–500 chars; `immediately` optional, default `false`) → `200 { status, currentPeriodEnd }`. Records the reason as a `MANUAL` `subscription_event` (payload `{ "reason": ..., "by": "ADMIN" }`) before calling Stripe; `immediately=false` schedules cancellation at period end (same as customer self-serve), `immediately=true` cancels the Stripe subscription right away. Either way the CANCELLED status itself is applied later by the `customer.subscription.deleted` webhook, not by this endpoint. Unknown subscriber → `404`; no Stripe subscription yet → `409 NO_BILLING_ACCOUNT`; not eligible to cancel → `409 ILLEGAL_STATE_TRANSITION` |
| `POST /api/admin/subscribers/{id}/pause` · `POST /api/admin/subscribers/{id}/resume` | → `200 { status, currentPeriodEnd }` — admin-triggered version of the customer self-serve pause/resume (same Stripe calls and guards): pause requires ACTIVE, resume requires PAUSED, else `409 ILLEGAL_STATE_TRANSITION`; unknown subscriber → `404`; no Stripe subscription yet → `409 NO_BILLING_ACCOUNT` |
| `GET /api/admin/subscribers/{id}/events` | subscriber activity history, newest first, capped at 100: `[{ id, type, source, occurredAt, note }]` — `source` is the `SubscriptionEventSource` (`STRIPE_WEBHOOK` / `MANUAL` / `SYSTEM`); `note` is the cancellation reason for `CANCELLATION_REQUESTED` events (extracted from the JSONB payload), `null` for every other event type. Unknown subscriber → `404` |
