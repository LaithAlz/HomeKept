...
| `GET /api/tech/properties/{propertyId}/notes` | this property's threaded operational notes, newest first, capped at 100 — same shape as the visit notes above, same `property_note` rows the admin console reads/writes via `GET`/`POST /api/admin/properties/{propertyId}/notes`. Standing context about the home ("dog in the yard", "gate sticks"), NOT a single visit's own notes. Property not found, OR this technician has never been assigned a visit at it (any status — not filtered to SCHEDULED/upcoming) → `404` (indistinguishable) |
...
| `GET /api/admin/properties/{propertyId}/notes` | the property's threaded operational notes, newest first, capped at 100: `[{ id, body, createdAt, authorUserId, authorFirstName, authorLastName }]` (`property_note` table, V18) — standing knowledge about the home ("dog in the yard", "gate sticks, lift while pushing"), as distinct from a single visit's own notes. Same rows the assigned technician reads/writes via `GET`/`POST /api/tech/properties/{propertyId}/notes`. Always plaintext — never the encrypted `access_notes` column. Unknown `propertyId` → 404 |
...
| `GET /api/admin/visits/{id}/notes` | the visit's threaded operational notes, newest first, capped at 100: `[{ id, body, createdAt, authorUserId, authorFirstName, authorLastName }]` — unlike the events log above, the author IS resolved to a name (batched, never per-row), since a note's whole point is a readable log of who observed what. Same rows the assigned technician reads/writes via `GET`/`POST /api/tech/visits/{id}/notes`. NOT the same thing as `completionNotes`/`materialsNotes` (single fields set once at completion) — this is a log, any number of entries. Unknown visit → `404` |
