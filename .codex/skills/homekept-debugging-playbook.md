...
> **When to use this:** Open this FIRST when something is broken and you need to
> narrow it fast: login/auth failing, email or photos not working, the backend
> won't start, a 403/404 you did not expect, a build failing, wrong money or
> time on screen. Triggers: "not working", "login fails", "no email", "photos
> 503", "won't start", "unexpected 403", "unexpected 404", "build fails", debug,
> triage, broken, stuck.
...
behind these live in `.codex/skills/homekept-failure-archaeology.md`; this is
the flowchart. Verified 2026-07-06.
...
| **Login works locally, fails deployed** | Is the API on `api.homekept.ca` (not `*.onrender.com`)? Is `APP_SECURE_COOKIES=true`? | Cookie is not same-site with the frontend, or not Secure behind TLS. See `.codex/skills/homekept-run-and-operate.md`. |
...
| **Email never arrives** | Is the env var `APP_SENDGRID_*` (not `SENDGRID_API_KEY`)? | Config-binding bug #120: no `app.sendgrid:` yml block, so `SENDGRID_API_KEY` binds to nothing and the sender logs-and-skips. `.codex/skills/homekept-config-and-flags.md`. |
...
| **Contrast/looks-wrong after a style change** | Did you change a token in `theme.css`? | Re-check WCAG AA; text on the amber accent is always the dark ink `#11201a`. (See the #122 regression.) |
...
2. **SSR has no cookie, so guards are client-side.** Do not try to authenticate
   ...
   the server. That is the architecture, not a bug
...
3. **Integrations fail silently by design.** SendGrid/R2/Stripe degrade
   ...
   warning and verify the env-var *name* against
...
- The full story/commit behind a fixed bug → `.codex/skills/homekept-failure-archaeology.md`.
- Measuring rather than guessing (tools, logs, curl recipes) →
  `.codex/skills/homekept-diagnostics-and-tooling.md`.
- Config/env specifics → `.codex/skills/homekept-config-and-flags.md`.
- Whether your fix needs a review gate → `.codex/skills/homekept-change-control.md`
  (and `AGENTS.md` for the workflow and human-only boundary).

## Provenance and maintenance

Verified 2026-07-06. Re-verify the referenced fixes with the commands in
`.codex/skills/homekept-failure-archaeology.md` (Provenance). Add a new row here
whenever a symptom costs more than an hour to diagnose.
