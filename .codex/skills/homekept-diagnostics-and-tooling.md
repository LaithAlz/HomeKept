...
> **When to use this:** Open this when you need evidence rather than a guess: is
> the API healthy, did a non-negotiable get violated, why did CI fail, what did a
> test actually assert. Triggers: "measure", "check the API", "curl", "CI logs",
> "gh run", "test report", "scan for violations", "is it actually working",
> diagnostics, instrumentation, smoke test.
...
```bash
.codex/skills/scripts/check-invariants.sh
```
...
`.codex/skills/homekept-architecture-contract.md`). Use the scan to find where
to look, then read the code around each hit.
...
See `.codex/skills/homekept-validation-and-qa.md` for the always-test rules.
...
- Narrowing a symptom to a cause → `.codex/skills/homekept-debugging-playbook.md`
  (this skill is the instruments; that one is the flowchart).
- Whether a change is *proven* / what to test → `.codex/skills/homekept-validation-and-qa.md`.
- First-principles proofs of an invariant → `.codex/skills/homekept-proof-and-analysis-toolkit.md`.
- Config/env var checks → `.codex/skills/homekept-config-and-flags.md`.
...
- Scripts run clean: `bash -n .codex/skills/scripts/*.sh` then run `check-invariants.sh`
