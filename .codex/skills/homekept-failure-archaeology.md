...
> **When to use this:** Open this when a symptom feels familiar, before "fixing"
> something that may already be understood, when reviewing auth/cookie/contrast/
> config code, or when you want the story behind a rule. Triggers: "has this
> happened before", "why did this break", "has login broken before", known login
> regression, empty body, open redirect, WCAG contrast, config not binding,
> DOCTYPE test, "known issue", past bug, regression, history.
...
- **Status:** FIXED. Lesson: a 2xx with an empty body is success; never assume a
  JSON body. Related trap in `.codex/skills/homekept-debugging-playbook.md`.
...
- **Evidence:** `application.yml`, `backend/.env.example`, `docs/go-live-checklist.md`
  §3. **Status:** OPEN — founder hand-write. Detail: `.codex/skills/homekept-config-and-flags.md`.
...
- Live triage of a symptom you are seeing now → `.codex/skills/homekept-debugging-playbook.md`
  (this skill is the archive; that one is the flowchart).
- The rule a fix hardened into → `.codex/skills/homekept-change-control.md`.
- Config-binding specifics → `.codex/skills/homekept-config-and-flags.md`.
