...
> **When to use this:** Open this when working on launch readiness, the
> config-binding blockers, the end-to-end conversion path, or asking "why can't we
> go live yet" — the executable, decision-gated campaign to take HomeKept from
> code-done to the first paying customer: numbered phases, exact commands, the
> expected observation at each gate and where to branch when you see something
> else, the ranked fix menu for the blockers, the wrong paths fenced off, and the
> promotion protocol through the change-control gates (go-live blockers, launch
> checklist, first customer, activate, checkout, webhook not firing, email not
> sending on prod, deploy readiness, conversion path, issues #120, #121, #89).
...
(`.codex/skills/homekept-change-control.md`). An engineer/agent can do
everything up to those gates: reproduce, propose the exact hand-write diff, and
verify end-to-end. The human executes the boundary steps. The founder checklist
of record is `docs/go-live-checklist.md`; this skill is the gated procedure.
Verified 2026-07-06.
...
EMAIL=<seed-admin> PASSWORD=<seed-pw> \
  .codex/skills/scripts/api-smoke.sh /api/app/account
```
...
server (`.codex/skills/homekept-debugging-playbook.md`).
...
Every code fix in this campaign routes through the normal review gates: build →
spec review → adversarial security review if it touches
auth/Stripe/webhooks/state → PR → green CI → human merge (see `AGENTS.md` and
`.codex/skills/homekept-change-control.md`). **Success is measured** (the gates
above), never judged by eye. Hand-write and deploy steps are the founder's; you
propose the exact diff and verify the result.

## When NOT to use this / open a sibling instead

- The static human checklist → `docs/go-live-checklist.md`.
- Topology and the cookie constraint in depth →
  `.codex/skills/homekept-run-and-operate.md`.
- The config keys and binding bugs in depth →
  `.codex/skills/homekept-config-and-flags.md`.
- A symptom mid-campaign → `.codex/skills/homekept-debugging-playbook.md`.
- Proving a gate really passed →
